...
import (
	"fmt"
	"github.com/claymodel/lua-parser/utils"
	"math"
	"reflect"
)
...
func sline(pos utils.PositionHolder) int {
	return pos.Line()
}

func eline(pos utils.PositionHolder) int {
	return pos.LastLine()
}
...
func isVarArgReturnExpr(expr utils.Expr) bool {
	switch ex := expr.(type) {
	case *utils.FuncCallExpr:
		return !ex.AdjustRet
	case *utils.Comma3Expr:
		return true
	}
	return false
}

func lnumberValue(expr utils.Expr) (LNumber, bool) {
	if ex, ok := expr.(*utils.NumberExpr); ok {
...
func newCodeBlock(localvars *varNamePool, blabel int, parent *codeBlock, pos utils.PositionHolder) *codeBlock {
...
func (fc *funcContext) EnterBlock(blabel int, pos utils.PositionHolder) {
...
func compileChunk(context *funcContext, chunk []utils.Stmt) { // {{{
	for _, stmt := range chunk {
		compileStmt(context, stmt)
	}
} // }}}

func compileBlock(context *funcContext, chunk []utils.Stmt) { // {{{
	if len(chunk) == 0 {
		return
	}
	ph := &utils.Node{}
...
func compileStmt(context *funcContext, stmt utils.Stmt) { // {{{
	switch st := stmt.(type) {
	case *utils.AssignStmt:
		compileAssignStmt(context, st)
	case *utils.LocalAssignStmt:
		compileLocalAssignStmt(context, st)
	case *utils.FuncCallStmt:
		compileFuncCallExpr(context, context.RegTop(), st.Expr.(*utils.FuncCallExpr), ecnone(-1))
	case *utils.DoBlockStmt:
		context.EnterBlock(labelNoJump, st)
		compileChunk(context, st.Stmts)
		context.LeaveBlock()
	case *utils.WhileStmt:
		compileWhileStmt(context, st)
	case *utils.RepeatStmt:
		compileRepeatStmt(context, st)
	case *utils.FuncDefStmt:
		compileFuncDefStmt(context, st)
	case *utils.ReturnStmt:
		compileReturnStmt(context, st)
	case *utils.IfStmt:
		compileIfStmt(context, st)
	case *utils.BreakStmt:
		compileBreakStmt(context, st)
	case *utils.NumberForStmt:
		compileNumberForStmt(context, st)
	case *utils.GenericForStmt:
		compileGenericForStmt(context, st)
	}
} // }}}

func compileAssignStmtLeft(context *funcContext, stmt *utils.AssignStmt) (int, []*assigncontext) { // {{{
	reg := context.RegTop()
	acs := make([]*assigncontext, 0, len(stmt.Lhs))
	for i, lhs := range stmt.Lhs {
		islast := i == len(stmt.Lhs)-1
		switch st := lhs.(type) {
		case *utils.IdentExpr:
		...
		case *utils.AttrGetExpr:
			...
			if _, ok := st.Key.(*utils.StringExpr); ok {
				ac.keyks = true
			}
			acs = append(acs, ac)
...
func compileAssignStmtRight(context *funcContext, stmt *utils.AssignStmt, reg int, acs []*assigncontext) (int, []*assigncontext) { // {{{
	...
	for namesassigned < lennames {
		...
		if ec.ctype == ecTable {
			if _, ok := expr.(*utils.LogicalOpExpr); !ok {
...
func compileAssignStmt(context *funcContext, stmt *utils.AssignStmt) { // {{{
	code := context.Code
	lennames := len(stmt.Lhs)
	...
	for i := lennames - 1; i >= 0; i-- {
		...
		case ecLocal:
			if acs[i].needmove {
				code.AddABC(OP_MOVE, context.FindLocalVar(ex.(*utils.IdentExpr).Value), reg, 0, sline(ex))
				reg -= 1
			}
		case ecGlobal:
			code.AddABx(OP_SETGLOBAL, reg, context.ConstIndex(LString(ex.(*utils.IdentExpr).Value)), sline(ex))
			reg -= 1
		case ecUpvalue:
			code.AddABC(OP_SETUPVAL, reg, context.Upvalues.RegisterUnique(ex.(*utils.IdentExpr).Value), 0, sline(ex))
			reg -= 1
		case ecTable:
			opcode := OP_SETTABLE
			if acs[i].keyks {
...
func compileRegAssignment(context *funcContext, names []string, exprs []utils.Expr, reg int, nvars int, line int) { // {{{
	lennames := len(names)
	lenexprs := len(exprs)
	namesassigned := 0
	ec := &expcontext{}
...
func compileLocalAssignStmt(context *funcContext, stmt *utils.LocalAssignStmt) { // {{{
	reg := context.RegTop()
	if len(stmt.Names) == 1 && len(stmt.Exprs) == 1 {
		if _, ok := stmt.Exprs[0].(*utils.FunctionExpr); ok {
...
func compileReturnStmt(context *funcContext, stmt *utils.ReturnStmt) { // {{{
...
func compileIfStmt(context *funcContext, stmt *utils.IfStmt) { // {{{
...
func compileBranchCondition(context *funcContext, reg int, expr utils.Expr, thenlabel, elselabel int, hasnextcond bool) { // {{{
	...
	switch ex := expr.(type) {
	case *utils.FalseExpr, *utils.NilExpr:
		if !hasnextcond {
			code.AddASbx(OP_JMP, 0, elselabel, sline(expr))
			return
		}
	case *utils.TrueExpr, *utils.NumberExpr, *utils.StringExpr:
		if !hasnextcond {
			return
		}
	case *utils.UnaryNotOpExpr:
		compileBranchCondition(context, reg, ex.Expr, elselabel, thenlabel, !hasnextcond)
		return
	case *utils.LogicalOpExpr:
		switch ex.Operator {
		case "and":
...
func compileWhileStmt(context *funcContext, stmt *utils.WhileStmt) { // {{{
	thenlabel := context.NewLabel()
	elselabel := context.NewLabel()
...
func compileRepeatStmt(context *funcContext, stmt *utils.RepeatStmt) { // {{{
...
func compileBreakStmt(context *funcContext, stmt *utils.BreakStmt) { // {{{
...
func compileFuncDefStmt(context *funcContext, stmt *utils.FuncDefStmt) { // {{{
	if stmt.Name.Func == nil {
		...
		astmt := &utils.AssignStmt{Lhs: []utils.Expr{stmt.Name.Func}, Rhs: []utils.Expr{stmt.Func}}
...
func compileNumberForStmt(context *funcContext, stmt *utils.NumberForStmt) { // {{{
	code := context.Code
	endlabel := context.NewLabel()
	ec := &expcontext{}
	...
	if stmt.Step == nil {
		stmt.Step = &utils.NumberExpr{Value: "1"}
...
func compileGenericForStmt(context *funcContext, stmt *utils.GenericForStmt) { // {{{
	code := context.Code
	endlabel := context.NewLabel()
...
func compileExpr(context *funcContext, reg int, expr utils.Expr, ec *expcontext) int { // {{{
	code := context.Code
	sreg := savereg(ec, reg)
	sused := 1
	if sreg < reg {
	...
	case *utils.AttrGetExpr:
		...
		opcode := OP_GETTABLE
		if _, ok := ex.Key.(*utils.StringExpr); ok {
			opcode = OP_GETTABLEKS
		}
		code.AddABC(opcode, a, b, c, sline(ex))
		return sused
	case *utils.TableExpr:
		compileTableExpr(context, reg, ex, ec)
		return 1
	case *utils.ArithmeticOpExpr:
		compileArithmeticOpExpr(context, reg, ex, ec)
		return sused
	case *utils.StringConcatOpExpr:
		compileStringConcatOpExpr(context, reg, ex, ec)
		return sused
	case *utils.UnaryMinusOpExpr, *utils.UnaryNotOpExpr, *utils.UnaryLenOpExpr:
...
func compileExprWithPropagation(context *funcContext, expr utils.Expr, reg *int, save *int, propergator func(int, *int, *int, int)) { // {{{
...
func compileExprWithKMVPropagation(context *funcContext, expr utils.Expr, reg *int, save *int) { // {{{
	compileExprWithPropagation(context, expr, reg, save, context.Code.PropagateKMV)
} // }}}

func compileExprWithMVPropagation(context *funcContext, expr utils.Expr, reg *int, save *int) { // {{{
	compileExprWithPropagation(context, expr, reg, save, context.Code.PropagateMV)
} // }}}

func constFold(exp utils.Expr) utils.Expr { // {{{
...
func compileFunctionExpr(context *funcContext, funcexpr *utils.FunctionExpr, ec *expcontext) { // {{{
...
func compileTableExpr(context *funcContext, reg int, ex *utils.TableExpr, ec *expcontext) { // {{{
	code := context.Code
	/*
	...
	*/
	...
	for i, field := range ex.Fields {
		...
		if field.Key == nil {
			...
			opcode := OP_SETTABLE
			if _, ok := field.Key.(*utils.StringExpr); ok {
				opcode = OP_SETTABLEKS
			}
...
func compileArithmeticOpExpr(context *funcContext, reg int, expr *utils.ArithmeticOpExpr, ec *expcontext) { // {{{
	exp := constFold(expr)
	...
	expr, _ = exp.(*utils.ArithmeticOpExpr)
	a := savereg(ec, reg)
	b := reg
...
func compileStringConcatOpExpr(context *funcContext, reg int, expr *utils.StringConcatOpExpr, ec *expcontext) { // {{{
	code := context.Code
	crange := 1
	for current := expr.Rhs; current != nil; {
		if ex, ok := current.(*utils.StringConcatOpExpr); ok {
			crange += 1
			current = ex.Rhs
		} else {
			current = nil
...
func compileUnaryOpExpr(context *funcContext, reg int, expr utils.Expr, ec *expcontext) { // {{{
	opcode := 0
	code := context.Code
	var operandexpr utils.Expr
	switch ex := expr.(type) {
	case *utils.UnaryMinusOpExpr:
		exp := constFold(ex)
		...
		ex, _ = exp.(*utils.UnaryMinusOpExpr)
		operandexpr = ex.Expr
		opcode = OP_UNM
	case *utils.UnaryNotOpExpr:
		switch ex.Expr.(type) {
...
func compileRelationalOpExprAux(context *funcContext, reg int, expr *utils.RelationalOpExpr, flip int, label int) { // {{{
	code := context.Code
	b := reg
	compileExprWithKMVPropagation(context, expr.Lhs, &reg, &b)
	c := reg
...
func compileRelationalOpExpr(context *funcContext, reg int, expr *utils.RelationalOpExpr, ec *expcontext) { // {{{
	a := savereg(ec, reg)
	code := context.Code
...
func compileLogicalOpExpr(context *funcContext, reg int, expr *utils.LogicalOpExpr, ec *expcontext) { // {{{
	a := savereg(ec, reg)
	code := context.Code
	endlabel := context.NewLabel()
...
func compileLogicalOpExprAux(context *funcContext, reg int, expr utils.Expr, ec *expcontext, thenlabel, elselabel int, hasnextcond bool, lb *lblabels) { // {{{
	// TODO folding constants?
	code := context.Code
	flip := 0
	jumplabel := elselabel
	...
	case *utils.TrueExpr:
		...
		return
	case *utils.NumberExpr, *utils.StringExpr:
		if thenlabel == lb.e {
			compileExpr(context, reg, expr, ec)
			code.AddASbx(OP_JMP, 0, lb.e, sline(expr))
		} else {
...
func compileFuncCallExpr(context *funcContext, reg int, expr *utils.FuncCallExpr, ec *expcontext) int { // {{{
	funcreg := reg
...
func loadRk(context *funcContext, reg *int, expr utils.Expr, cnst LValue) int { // {{{
...
func getIdentRefType(context *funcContext, current *funcContext, expr *utils.IdentExpr) expContextType { // {{{
	if current == nil {
		return ecGlobal
...
func getExprName(context *funcContext, expr utils.Expr) string { // {{{
	switch ex := expr.(type) {
	case *utils.IdentExpr:
		return ex.Value
	case *utils.AttrGetExpr:
...
func Compile(chunk []utils.Stmt, name string) (proto *FunctionProto, err error) { // {{{
	defer func() {
	...
	err = nil
	parlist := &utils.ParList{HasVargs: true, Names: []string{}}
	funcexpr := &utils.FunctionExpr{ParList: parlist, Stmts: chunk}
